...
GET:
	...
	{
		dataMode = t_datamode;
		cout << "set PsDepth_30,datemode index:" << t_datamode << endl;
	}
	...
	if (status != PsRetOK)
	{
		cout << "PsDepthAndIR_30  status" << status << endl;
	}
	else
	{
		dataMode = t_datamode;
		cout << "set PsDepthAndIR_30,datemode index:" << t_datamode << endl;
	}

	//1.ReadNextFrame.
	//2.Get Depth and IR Frame.
	for (int i = 0; i < frameSpace; i++)
	{
		status = Ps2_ReadNextFrame(deviceHandle, sessionIndex, &FrameReady);
		if (status != PsReturnStatus::PsRetOK)
		...
		if (1 == FrameReady.depth && 1 == FrameReady.ir)
		{
			status = Ps2_GetFrame(deviceHandle, sessionIndex, PsDepthFrame, &depthFrame);
			if (depthFrame.pFrameData != NULL)
			{
