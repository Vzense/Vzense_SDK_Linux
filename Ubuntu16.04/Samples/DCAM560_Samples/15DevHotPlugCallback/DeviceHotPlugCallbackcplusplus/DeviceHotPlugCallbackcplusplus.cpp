...
public:
	...
	static void HotPlugStateCallback_(const PsDeviceInfo* pInfo, int status, void* contex)
	{
	...
	int registCallback()
	{
		return Ps2_SetHotPlugStatusCallback_(HotPlugStateCallback_, this);
	}

	void HandleCallback(const PsDeviceInfo* pInfo, int status)
	{
...
GET:
	...
	{
		Sensor s;
		status = (PsReturnStatus)s.registCallback();
		if (status != PsReturnStatus::PsRetOK)
		{
