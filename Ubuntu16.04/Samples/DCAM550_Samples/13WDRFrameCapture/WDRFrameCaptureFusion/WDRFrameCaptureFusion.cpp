...
GET:
	...
	//3.set WDRStyle to fusion(fusion or alternations of ranges)
	//4.ReadNextFrame.
	...
	status = Ps2_SetWDRStyle(deviceHandle, sessionIndex, PsWDR_FUSION);
	if (PsRetOK == status)
	{
		cout << "WDR image output Fusion." << endl;
	}
	else
	{
	...
	for (int i = 0; i < frameSpace; i++)
	{
		...
		if (1 == FrameReady.wdrDepth) {
			status = Ps2_GetFrame(deviceHandle, sessionIndex, PsWDRDepthFrame, &wdrDepth);
			if (wdrDepth.pFrameData != NULL)
			{
				cout << "get Frame successful,status:" << status << "  "
					<< "frameTpye:" << wdrDepth.frameType << "  "
					<< "frameIndex:" << wdrDepth.frameIndex << endl;
			}
			else
			{
				cout << "no frame";
